...
To future-proof the Template Service we've introduced a new major version update, which includes several structural and syntax changes. [The Template Service documentation](../services/other/template-service/) is already updated, but this guide will help you understand the changes and how to migrate your existing v1 templates to the new v2 format.
...
## Recommended Migration Strategy <a href="#recommended-migration-strategy" id="recommended-migration-strategy"></a>

We've worked on making this as easy as possible with the most recent changes to the Extra Horizon CLI. So we'll start by recommending [installing the latest version of the CLI](https://docs.extrahorizon.com/cli/readme/installation) to help you with the migration.
...
## Duplicate Localizations with parameters <a href="#duplicate-localizations-with-parameters" id="duplicate-localizations-with-parameters"></a>

If you are using localizations with parameters in your templates, you will need to update them. [The new localization format](../services/other/template-service/localizations.md) is not compatible with the old one.
...
## Migrate Templates <a href="#migrate-templates" id="migrate-templates"></a>
...
If you are not already using the CLI to manage your templates, we highly recommend doing so. See the CLI documentation for [more information on how to use the CLI to manage your templates](https://docs.extrahorizon.com/cli/commands/templates). You can also commit these template files to your version control system to keep track of (future) changes.

Furthermore, if you maintain multiple environments (such as development, staging, and production), you may need certain email templates to differ between them (for example, using environment-specific links). To address this, the CLI now supports `variables` for v2 templates. This feature lets you define variables within your templates and [assign different values for each environment by providing environment variables to the CLI](https://docs.extrahorizon.com/cli/commands/templates#using-environment-variables-in-variables).
...
### Structural Changes <a href="#structural-changes" id="structural-changes"></a>

#### Rename `schema.fields` → `inputs` <a href="#rename-schemafields--inputs" id="rename-schemafields--inputs"></a>
...
#### Rename `fields` → `outputs` <a href="#rename-fields--outputs" id="rename-fields--outputs"></a>
...
#### Update the `object` type definitions <a href="#update-the-object-type-definitions" id="update-the-object-type-definitions"></a>
...
#### Update the `array` type definitions <a href="#update-the-array-type-definitions" id="update-the-array-type-definitions"></a>
...
#### Remove all `options` arrays <a href="#remove-all-options-arrays" id="remove-all-options-arrays"></a>
...
#### Replace deprecated types <a href="#replace-deprecated-types" id="replace-deprecated-types"></a>
...
### Templating Syntax Changes <a href="#templating-syntax-changes" id="templating-syntax-changes"></a>
...
Check out the main documentation for more information on [the new syntax and handlebars helpers that are available in the v2 templates](../services/other/template-service/helpers-in-templates.md), but below we will highlight the most important changes that you need to be aware of when migrating your templates.

#### Input Access <a href="#input-access" id="input-access"></a>
...
#### Localizations <a href="#localizations" id="localizations"></a>
...
## Update cluster settings <a href="#update-cluster-settings" id="update-cluster-settings"></a>
...
## Update application code <a href="#update-application-code" id="update-application-code"></a>
...
{% hint style="warning" %}
To have more control over who is allowed to use v2 templates, we also introduced a new permission, `RESOLVE_TEMPLATES`. So if your code directly resolves a template, you will need to make sure that the user who is making the request has this permission.
{% endhint %}
...
When using the Template Service API directly, similarly you can update the endpoints used from `/templates/v1/` to `/templates/v2/` and update the request body to match [the new structure of resolving v2 templates](https://swagger.extrahorizon.com/swagger-ui/index.html?url=https://swagger.extrahorizon.com/templates-service/2.0.0/openapi.yaml#/Template/post__templateId__resolve).

## Test thoroughly and Remove V1 Templates <a href="#test-thoroughly-and-remove-v1-templates" id="test-thoroughly-and-remove-v1-templates"></a>
